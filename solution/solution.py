...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(entry_averages) > 1:
            first, rest = entry_averages[0], entry_averages[1:]
            positional_bias = all(first > other for other in rest)
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter claims unsupported by retrieved context"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Improve prompt clarity and query understanding so answers directly address question intent"
            )
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection and system prompt routing to keep answers on-topic"
            )
        ...
        if len(suggestions) < 3:
            suggestions.append(
                "Expand golden dataset coverage for the observed failure patterns and re-run regression"
            )
